...
def fetchAllStudentInDB():
    sql = "select * from kruskal"
...
Kruskalgraph = nx.from_pandas_edgelist(df2, source='Source Node', target='Destination Node', edge_attr=True)
pos = nx.circular_layout(Kruskalgraph)
nx.draw(Kruskalgraph,pos,with_labels=True)
nx.draw_networkx_edge_labels(Kruskalgraph,pos,edge_labels=minLables)
plt.show()